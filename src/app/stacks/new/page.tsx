...
export default function NewStackPage() {
  ...
  const [picked, setPicked] = useState<Film[]>([]);
  ...
  const publish = useCallback(async () => {
    if (publishing || !name.trim() || picked.length === 0) return;
    setPublishing(true);
    setError("");
    try {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-8">
      <div className="space-y-2">
        <p className="text-xs uppercase tracking-wider text-amber-500 font-medium">New stack</p>
        <h1 className="text-3xl sm:text-4xl font-bold text-white">Create a stack</h1>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button
            type="button"
            onClick={publish}
            disabled={publishing || !name.trim() || picked.length === 0}
